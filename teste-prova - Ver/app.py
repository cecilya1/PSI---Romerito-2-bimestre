...
def mostrar_usuario():
    ...
    conn.close()
    return render_template('mostrar_usuario.html', lista=resultado)
...
def cadastrar_peca():
    if request.method == 'POST':
        ...
        conn = obter_conexao()
        sql = 'SELECT * FROM tb_pecas WHERE pec_nome = ? AND pec_turma = ?'
        resultado = conn.execute(sql, (nome, turma)).fetchone()
        if not resultado:
            sql2 = 'INSERT INTO tb_pecas(pec_nome, pec_turma) VALUES (?, ?)'
            conn.execute(sql2, (nome, turma))
            conn.commit()
            conn.close()
            return redirect(url_for('index'))
        flash('Essa peça já está cadastrada com esses dados', category='erro')
...
def buscar_peca():
    if request.method == 'POST':
        nome = request.form['nome']
        conn = obter_conexao()
        resultado2 = conn.execute('SELECT * FROM tb_pecas WHERE pec_nome = ?', (nome,)).fetchall()
        if resultado2:
            return render_template('mostrar_peca.html', lista = resultado2)
...
@app.route('/listar_dancas', methods = ['POST', 'GET'])
def listar_dancas():
    conn = obter_conexao()
    sql = 'SELECT * FROM tb_dancas'
    resultado = conn.execute(sql).fetchall()
    conn.close()
    return render_template('listar_dancas.html', lista=resultado)

@app.route('/mostrar_danca')
def mostrar_danca():
    id = request.args.get("id")
    conn = obter_conexao()
    sql = 'SELECT * FROM tb_dancas WHERE dan_id = ?'
    resultado = conn.execute(sql, (id,)).fetchall()
    conn.close()
    return render_template('mostrar_danca.html', lista=resultado)

@app.route('/buscar_danca', methods = ['POST', 'GET'])
def buscar_danca():
    if request.method == 'POST':
        id = request.form['id']
        conn = obter_conexao()
        resultado2 = conn.execute('SELECT * FROM tb_dancas WHERE dan_id = ?', (id,)).fetchall()
        if resultado2:
            return render_template('mostrar_danca.html', lista = resultado2)
        flash('Não existe nenhuma dança cadastrada com esses dados', category='erro')
        return redirect(url_for('buscar_danca'))
    return render_template('buscar_danca.html')
...
def mostrar_peca():
    ...
    resultado = conn.execute(sql, (id,)).fetchall()
    conn.close()
    return render_template('mostrar_peca.html', lista=resultado, qnt=len(resultado))
...
def editar_peca():
    if request.method == 'POST':
        ...
        id = request.form['id']
        conn = obter_conexao()
        sql = 'UPDATE tb_pecas SET pec_nome = ?, pec_turma = ? WHERE pec_id = ?'
    ...
    resultado = conn.execute(sql, (id,)).fetchone()
    conn.close()
    print(resultado['pec_nome'])
    return render_template('editar_peca.html', resultado=resultado)
